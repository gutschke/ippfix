...
# ---------------------------------------------------------------------------
# Splitting a job that is too large to send in one piece.
#
# Outlining inlines a path at every glyph occurrence, so a long document grows
# past what the printer will accept as a PDF -- about 155 ordinary report pages
# on the device this was built for. The raster fallback does not rescue it: the
# raster is larger still and crosses the proxy's own ceiling twenty pages later,
# after which the original is relayed with its fonts intact, which is the one
# outcome this proxy exists to prevent.
#
# So the job is sent as several jobs whose pages add up to the original, and the
# client is shown one. Everything below is about doing that without ever putting
# a wrong page on paper -- this is the first thing here that can print something
# incorrect rather than merely fail to print, and the whole design leans on
# refusing when it cannot be sure.
# ---------------------------------------------------------------------------

# How much of the printer's declared limit a planned chunk may aim for. The plan
# is made from an average, and pages are not uniform: a chapter opening with a
# display face costs many times an ordinary page. Chunks are verified after
# conversion and re-split if they overshoot, so this margin only decides how
# often that second pass is needed -- but a generous margin is much cheaper than
# a re-split, and far cheaper than a chunk the printer refuses.
SPLIT_MARGIN = 0.80
# Never plan more than this many chunks. A document that would need more is
# beyond what this mechanism should be attempting; refusing is better than
# holding the printer for an hour.
MAX_CHUNKS = 32


class NotSplittable(Exception):
    """Why this particular job must not be split.

    Carries the reason so it can be logged and put in a report: an operator who
    sees a job fall back to the old behaviour needs to know which attribute
    caused it, or the refusal looks like a fault.
    """


def splittable(group):
    """Check a job's attributes for anything that splitting would corrupt.

    Splitting is invisible to a client only when every page is independent of
    every other. These attributes break that assumption, each in its own way,
    and none of them can be verified correct without printing the result -- so
    they are refused rather than guessed at. Raises NotSplittable.
    """
    if group is None:
        raise NotSplittable('the job carries no operation attributes')

    copies = group.get_int('copies')
    if copies is not None and copies > 1:
        # Sending each chunk N times yields 1,1,1,2,2,2 rather than three
        # collated sets, and there is no way to collate across separate jobs on
        # a printer that will not take several documents in one.
        raise NotSplittable(f'copies={copies}; collation cannot survive '
                            f'being split across separate jobs')

    if group.index_of('page-ranges') >= 0:
        # page-ranges names pages of the ORIGINAL document. Applied again to a
        # chunk that has already been reduced to pages 40-60, it selects the
        # wrong pages -- and prints them, which is worse than printing nothing.
        raise NotSplittable('page-ranges is present and numbers the original '
                            'document, which a chunk no longer is')

    if group.index_of('overrides') >= 0:
        # Same problem: overrides carry original page numbers.
        raise NotSplittable('overrides are present and number the original '
                            'document')

    n_up = group.get_int('number-up')
    if n_up is not None and n_up > 1:
        # Pages are grouped onto sheets, so a split point inside a group would
        # leave a partly filled sheet and shift every group after it.
        raise NotSplittable(f'number-up={n_up}; a split point inside a group '
                            f'would shift every sheet after it')

    finishings = group.get('finishings')
    if finishings:
        # 3 is 'none'. Anything else is applied per job, so a stapled job would
        # come back as several separately stapled booklets -- and unlike a
        # wrong page, that cannot be undone by reprinting.
        values = [struct.unpack('>i', v)[0] if len(v) == 4 else None
                  for v in finishings]
        if any(v != 3 for v in values):
            raise NotSplittable(f'finishings={values} would be applied to '
                                f'every chunk separately')

    sheets = group.get_str('job-sheets')
    if sheets and sheets != 'none':
        # A banner page per chunk instead of per job.
        raise NotSplittable(f'job-sheets={sheets!r} would print once per chunk')

    handling = group.get_str('multiple-document-handling')
    if handling and handling not in ('single-document',
                                     'single-document-new-sheet'):
        raise NotSplittable(f'multiple-document-handling={handling!r}')

    sides = group.get_str('sides') or 'one-sided'
    if sides not in ('one-sided', 'two-sided-long-edge', 'two-sided-short-edge'):
        raise NotSplittable(f'unrecognised sides={sides!r}')
    return sides


def plan_split(total_pages, total_bytes, limit, duplex):
    """Chunk boundaries for a document of `total_pages` and `total_bytes`.

    Returns a list of inclusive 1-based (first, last) page ranges that tile
    1..total_pages exactly once, in order, with no gaps and no overlaps. That
    property is the whole point of this function and is asserted by the tests:
    a gap loses a page and an overlap prints one twice, and neither is visible
    to anyone until somebody reads the paper.

    When `duplex`, every chunk but the last has an even length. A chunk is a
    separate job and therefore starts on the front of a fresh sheet, so an
    odd-length chunk leaves a blank back and shifts the front/back pairing of
    every page after it -- the document still contains every page, in order,
    and is still wrong. An odd final chunk is fine: its last sheet has a blank
    back, which is exactly what the unsplit job would have produced.

    Raises NotSplittable if the document cannot be divided within MAX_CHUNKS,
    or if duplex parity and the size limit cannot both be satisfied.
    """
    if total_pages < 1:
        raise NotSplittable('the document has no pages')
    if total_bytes <= limit:
        return [(1, total_pages)]

    step = 2 if duplex else 1
    budget = max(1, int(limit * SPLIT_MARGIN))
    per_page = max(1, total_bytes // total_pages)
    per_chunk = int(budget // per_page)
    # Round down to a whole number of sheets, and never to nothing.
    per_chunk -= per_chunk % step
    if per_chunk < step:
        # One sheet already exceeds the limit. Splitting cannot help: the
        # smallest chunk this can produce is still too large.
        raise NotSplittable(
            f'a single {"sheet" if duplex else "page"} is already larger than '
            f'the printer will accept ({per_page:,} bytes per page against a '
            f'{limit:,} byte limit)')

    chunks = []
    first = 1
    while first <= total_pages:
        last = min(first + per_chunk - 1, total_pages)
        chunks.append((first, last))
        first = last + 1
    if len(chunks) > MAX_CHUNKS:
        raise NotSplittable(
            f'{len(chunks)} chunks would be needed, more than the {MAX_CHUNKS} '
            f'this will attempt')
    return chunks


def resplit(chunk, actual_bytes, limit, duplex):
    """Divide one chunk that turned out larger than planned.

    The plan is made from an average and pages are not uniform, so a chunk can
    overshoot however careful the margin was. Splitting it again on measured
    rather than averaged size is the correction; the alternative is sending a
    chunk the printer refuses, having already printed the ones before it.
    """
    first, last = chunk
    pages = last - first + 1
    if pages <= (2 if duplex else 1):
        raise NotSplittable(
            f'pages {first}-{last} still exceed the limit and cannot be '
            f'divided further')
    plan = plan_split(pages, actual_bytes, limit, duplex)
    if len(plan) < 2:
        # The planner would not divide it, but we know it is too large, so
        # halving on sheet boundaries is the honest fallback.
        step = 2 if duplex else 1
        half = max(step, (pages // 2) - ((pages // 2) % step))
        plan = [(1, half), (half + 1, pages)]
    return [(first + a - 1, first + b - 1) for a, b in plan]

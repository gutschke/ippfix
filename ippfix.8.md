...
* `--no-raster-retry`:
  ...
  page either arrives or it does not, and a raster of it is a page. This is
  what RFC 8011 and the IPP maintainers recommend for the case.

  It never reprints a page that already came out. Faults of this kind are often
  per page: a printer can render fourteen pages, meet something it cannot read
  on the fifteenth and abandon the job with fourteen sheets already in the
  tray. Before sending anything the proxy establishes that nothing was marked
  -- the printer's own impression count must be zero, and where the page
  counter tied to the marking engine can be read at both ends of the job, it
  must not have moved. If either says otherwise the document is left alone and,
  with `--alert-mail`, reported as part printed.
